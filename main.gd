...
func _ready():	
	bordoSup = get_node("/root/Parent/PlayScn/BordoSopra") # alternative: self.get_child(4) 
	nave = get_node("/root/Parent/PlayScn/Nave") # alternative:  self.get_child(3)
	# Preload cube
...
func _fixed_process(delta):
	...
	if spawn == true:	
		...
		if counter>spawnSpeed && spawn == true:
		...
		if nave.countToDeath >=5:						# Check if the ship hasn't moved in the last 5s
			...
			spawn = false #self.set_fixed_process(false)				# Stop updating _fixed_process(), so this is the last run
			
func reset(): # reset
	counter = 0 # timer for spawns
	distance = 0 # space traveled
	startPos = 0 # starting position of the ship
	endPos = 0 # final position of the ship
	spawnSpeed = 1.5 # spawn speed
	spawn = true